...
import { dagatalSubSk } from '../../shared/keys'
...
export const handler = async (
  ...
  try {
    ...
    if (!id) return toApiGatewayResponse(badRequest('Missing dagatal subscription id'))
    ...
    if (err.name === 'ConditionalCheckFailedException') {
      return toApiGatewayResponse(notFound('Dagatal subscription not found'))
    }
    console.error(err)
