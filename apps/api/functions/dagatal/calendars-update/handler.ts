...
import { dagatalSk } from '../../shared/keys'
...
export const handler = async (
  ...
  try {
    ...
    if (!id) return toApiGatewayResponse(badRequest('Missing dagatal calendar id'))

    const body = JSON.parse(event.body ?? '{}')
    const pk = getPk(event)
    const sk = dagatalSk(id)

    const existing = await dynamo.send(new GetCommand({ TableName: TABLE_NAME, Key: { pk, sk } }))
    if (!existing.Item) return toApiGatewayResponse(notFound('Dagatal calendar not found'))
    ...
    if (err.name === 'ConditionalCheckFailedException') {
      return toApiGatewayResponse(notFound('Dagatal calendar not found'))
    }
    console.error(err)
