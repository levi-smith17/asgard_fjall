...
const DEFAULT_WEB_URL = 'https://asgard.levismith.us'
...
export async function createContactSignal(
  ...
  const webUrl = (process.env.WEB_URL ?? DEFAULT_WEB_URL).replace(/\/$/, '')
  ...
  if (wayfarerEmail && process.env.SES_FROM_EMAIL) {
    await ses.send(new SendEmailCommand({
      ...
      Content: {
        Simple: {
          Subject: { Data: 'New message on Asgard', Charset: 'UTF-8' },
          Body: {
            Text: {
              Data: [
                ...
                input.body,
                '',
                `View and reply in Asgard: ${webUrl}/sendibod`,
