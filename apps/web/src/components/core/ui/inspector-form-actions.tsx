  ...
  return (
    <>
      <div className={cn('shrink-0 border-t border-border px-5 py-4', className)}>
        <Button type="button" size="sm" className="w-full" disabled={!canSave || isSaving} onClick={onSave}>
          {isSaving ? 'Saving…' : isNew ? createLabel : saveLabel}
        </Button>
      </div>
      {!isNew && showDelete && onDelete ? (
        <div className={cn('shrink-0 border-t border-border px-5 py-4', className)}>
          <Button type="button" size="sm" variant="outline" className="w-full border-destructive/40 text-destructive hover:bg-destructive/10" onClick={onDelete}>
            {deleteLabel}
          </Button>
        </div>
      ) : null}
