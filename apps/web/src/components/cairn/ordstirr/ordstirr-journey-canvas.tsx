...
function JourneySection({
  ...
  title: string
  registerSection: (id: OrdstirrJourneySectionId, node: HTMLElement | null) => void
  onSectionSelect: (id: OrdstirrJourneySectionId) => void
  onAdd?: () => void
  ...
  return (
    <section ref={ref} id={`ordstirr-journey-${id}`} className="flex scroll-mt-4 flex-col gap-4">
...
export function OrdstirrJourneyCanvas({
  data,
  terms,
  selectedEntryId,
  registerSection,
  ...
  onSectionSelect: (sectionId: OrdstirrJourneySectionId) => void
  onAddEntry: (sectionId: OrdstirrJourneySectionId) => void
  ...
  return (
    <div className="h-full overflow-y-auto" data-ordstirr-journey-scroll>
      <div className="mx-auto flex max-w-3xl flex-col gap-12 px-6 py-8">
        ...
        {passedCompanions.length > 0 ? (
          ...
          <JourneySection
            ...
            onAdd={() => onAddEntry('in-memoriam')}
          >
            <p className="text-sm text-muted-foreground">No {terms.summit_reached.toLowerCase()} entries.</p>
          </JourneySection>
        )}
      </div>
    </div>
