...
export interface NidjatalParentUpdate {
  fatherId?: string
...
export interface NidjatalKinNodeData {
  kin: NidjatalKin & { id: string }
  isValid: boolean
  isHighlighted: boolean
  allKin: (NidjatalKin & { id: string })[]
  onEdit: () => void
  onQuickParentFix: (update: NidjatalParentUpdate) => Promise<void>
}
...
export function kinFullName(kin: Pick<NidjatalKin, 'givenName' | 'middleName' | 'surname'>): string {
  return [kin.givenName, kin.middleName, kin.surname].filter(Boolean).join(' ')
}

export function kinDisplayName(
  kin: Pick<NidjatalKin, 'givenName' | 'middleName' | 'surname' | 'nickname'>,
): string {
...
export function getDescendantIds(
  ...
  for (const k of allKin) {
    if (k.fatherId) {
      const list = childrenOf.get(k.fatherId) ?? []
      list.push(k.id)
      childrenOf.set(k.fatherId, list)
    }
    if (k.motherId) {
      const list = childrenOf.get(k.motherId) ?? []
      list.push(k.id)
      childrenOf.set(k.motherId, list)
    }
  }
...
interface QuickParentFixProps {
  kin: NidjatalKin & { id: string }
  allKin: (NidjatalKin & { id: string })[]
  onSave: (update: NidjatalParentUpdate) => Promise<void>
}
...
function QuickParentFix({ kin, allKin, onSave }: QuickParentFixProps) {
  ...
  const eligibleParents = allKin.filter(
    (k) => k.id !== kin.id && !descendantIds.has(k.id) && isEligibleParent(k, kin),
  )
    ...
    <div className="absolute top-1.5 right-1.5 z-10" onClick={(e) => e.stopPropagation()}>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            className="flex h-5 w-5 items-center justify-center rounded-full border border-destructive/30 bg-destructive/10 text-destructive transition-colors hover:bg-destructive/20"
            title="Set parents"
          >
        ...
        <PopoverContent className="w-72 p-3" align="end" side="left">
          <p className="mb-3 text-xs font-medium">Set Parents</p>
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.checked) setFatherId('')
                }}
              />
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.checked) setMotherId('')
                }}
              />
              <span className="text-xs text-muted-foreground">Unknown</span>
            </label>
...
export const NidjatalKinNode = memo(function NidjatalKinNode({ data }: NodeProps<NidjatalKinNodeData>) {
  ...
  return (
    ...
    >
      ...
      {!isValid ? <QuickParentFix kin={kin} allKin={allKin} onSave={onQuickParentFix} /> : null}

      <div className="px-3 py-2.5">
        <div className="flex min-w-0 items-center gap-1.5">
          <User className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
          <p className="truncate text-sm font-medium leading-tight">{kinDisplayName(kin)}</p>
        </div>
        {dateLabel ? <p className="mt-0.5 pl-5 text-xs text-muted-foreground">{dateLabel}</p> : null}
        {currentBloodline ? (
          <div className="mt-1 flex items-center gap-1 pl-5">
            <Heart className="h-2.5 w-2.5 shrink-0 text-rose-400" />
            <span className="truncate text-[10px] text-muted-foreground">
              {currentBloodline.kinName}
              {currentBloodline.startDate ? ` (m. ${currentBloodline.startDate.slice(0, 4)})` : ''}
            </span>
          </div>
        ) : null}
      </div>
